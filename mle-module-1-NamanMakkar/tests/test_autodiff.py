...
def test_topsort() -> None:
    ...
    var4 = Function1.apply(0, var3)

    vars = minitorch.topological_sort(var4)
    ids = [var.unique_id for var in vars]

    assert len(ids) == len(set(ids))

    for idx, var in enumerate(vars):
        parents = var.history.inputs
        for parent in parents:
            parent_idx = ids.index(parent.unique_id)
